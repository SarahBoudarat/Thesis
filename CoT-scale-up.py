...
log("EXPERIMENT SCALE-UP: Chain-of-Thought prompting for 100 users (≥50 ratings each).")
...
for user_id in sampled_users_100:
    top_movies = train_data_100[train_data_100["userId"] == user_id].sort_values(by="rating", ascending=False).head(50)["title"].tolist()
    if len(top_movies) < 2:
        continue
