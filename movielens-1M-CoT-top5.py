...
log("EXPERIMENT MovieLens 1M: Chain-of-Thought prompting for 100 users (≥100 ratings each).")
...
for user_id in sampled_users_100:
    user_movies = train_data_100[train_data_100["userId"] == user_id].sort_values(by="timestamp")["title"].tolist()
    top_movies = user_movies[-50:]  # most recent 50
    if len(top_movies) < 2:
        continue
    ...
    prompt_text = (
        ...
        f"Movie Title: <Title 4>\n"
        f"Movie Title: <Title 5>"
    )
...
pd.DataFrame(prompt_logs).to_csv(os.path.join(output_dir, "prompt_log_4.csv"), index=False)
...
]).to_csv(os.path.join(output_dir, "raw_recommendations_4.csv"), index=False)
...
pd.DataFrame.from_dict(evaluation_results, orient="index").reset_index().rename(columns={"index": "user_id"}).to_csv(os.path.join(output_dir, "user_level_metrics_4.csv"), index=False)
pd.DataFrame(rows_for_csv).to_csv(os.path.join(output_dir, "recommendation_analysis_4.csv"), index=False)
