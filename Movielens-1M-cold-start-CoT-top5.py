...
prompt_type = "cold_start_chain_of_thought_top5"
...
# -----------------------------
# CHAIN-OF-THOUGHT PROMPT (Pure CoT, No Examples, Top-5)
# -----------------------------
...
for _, row in top_movies_per_user.iterrows():
    ...
    prompt = construct_cot_prompt(user_movies, user_id)  #  pass user_id
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.7,
            max_tokens=500  # increased for reasoning + output
        )
        text = response.choices[0].message.content.strip()

        # Extract lines in the "Movie Title: ..." format
        recs = [
...
# Save prompt logs
pd.DataFrame(prompt_logs).to_csv(f"{output_dir}/prompt_log_4_top5.csv", index=False)
...
]).to_csv(f"{output_dir}/raw_recommendations_4_top5.csv", index=False)
...
pd.DataFrame.from_dict(evaluation_results, orient="index").reset_index().rename(columns={"index": "user_id"}).to_csv(f"{output_dir}/user_level_metrics_4_top5.csv", index=False)
pd.DataFrame(recommendation_analysis).to_csv(f"{output_dir}/recommendation_analysis_4_top5.csv", index=False)

# Print summary
print("\nAverage Evaluation Metrics (Cold-Start Chain-of-Thought - Top-5):")
