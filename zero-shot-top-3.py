...
log("EXPERIMENT SCALE-UP: Zero-Shot Top-3 prompting for 100 users.")
...
df_prompt_log.to_csv(os.path.join(output_dir, "prompt_log_4_top3.csv"), index=False)
...
df_recs.to_csv(os.path.join(output_dir, "raw_recommendations_4_top3.csv"), index=False)
...
def evaluate_exact(recommendations, test_data, movies, verbose=True):
    ...
    for user_id, recs in recommendations.items():
        ...
        test_clean = [normalize(title) for title in test_titles]
        recs_clean = [normalize(title) for title in recs]

        hits = [recs[i] for i in range(len(recs_clean)) if recs_clean[i] in test_clean]
        in_dataset_count = sum([1 for title in recs_clean if title in known_titles])

        evaluation_results[user_id] = {
            "Hit@3": int(len(hits) > 0),
            "Precision@3": len(hits) / k,
            "Recall@3": len(hits) / len(test_clean),
            "NDCG@3": (
                sum([1 / np.log2(idx + 2) for idx, title in enumerate(recs_clean) if title in test_clean]) /
                sum([1 / np.log2(i + 2) for i in range(min(len(test_clean), k))])
            ) if min(len(test_clean), k) > 0 else 0.0,
            "InDataset@3": in_dataset_count / k
        }

        for i, raw_title in enumerate(recs):
            norm_title = recs_clean[i]
            label = (
                "match" if norm_title in test_clean else
            ...
            rows_for_csv.append({
                ...
                "label": label,
                "in_test_set": norm_title in test_clean,
                "in_movie_lens": norm_title in known_titles,
                "test_titles_clean": "; ".join(test_clean)
            })
...
if evaluation_results:
    log("\nAverage Evaluation Metrics (Exact Match - Top-3):")
    summary = {}
